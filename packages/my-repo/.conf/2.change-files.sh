...
mkdir -p "$package_dir/files/etc/apt/sources.list.d/" "$package_dir/files/etc/apt/keyrings/" && \
  cp "$pubkey" "$package_dir/files/etc/apt/keyrings/my-repo.gpg" || exit 1

cat << EOF > "$package_dir/files/etc/apt/sources.list.d/my-repo.list"
deb [arch=amd64 signed-by=/etc/apt/keyrings/my-repo.gpg] http://tegho.github.io/small_office_vpn/apt-repo stable main
EOF
