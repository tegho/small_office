...
monitoring_user="prometheus-client"
export_monitoring_file="$current_dir/prometheus-client.zip"
...
if [ -s "$pki_pwd_file" ] && [ -s "$container_file" ] ; then
  chmod -f 600 "$container_file" "$pki_pwd_file"
  ...
  # create new monitoring client cert
  crypt-pki "$container_file" --newreq --internalname "$monitoring_user" --nopass && \
    EASYRSA_PASSIN="file:$ca_pwd_file" crypt-pki "$container_file" --signserver --internalname "$monitoring_user" && \
    crypt-pki "$container_file" --export-zip --internalname "$monitoring_user" --file "$export_monitoring_file" || ret=1
